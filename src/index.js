...
export const onPreBuild = async function ({
  ...
  if (projectId == null) {
    return build.failBuild("ChiselStrike projectId not found. Add it to the plugin inputs.")
  }

  const csBuild = await getChiselStrikeBuildResult(projectId, process.env.COMMIT_REF)
  ...
  if (csBuild == "ERR") {
    return build.failBuild("ChiselStrike build failed. Interrupting Netlify build.")
  }

  return build.failBuild("ChiselStrike build not found. Interrupting Netlify build.")
}
...
async function getDeployStatus(projectId, commitRef) {
  ...
  const data = await axios.get(url).then(r => r.data).catch(e => [])
  if (data.length == 0) {
    return "UNKNOWN"
  }
